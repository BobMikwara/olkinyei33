...
import type { AdminUser, ModuleKey, SiteSettings } from "./types";
import { Lock } from "lucide-react";

// Modules
import PagesManager from "./modules/Pages";
...
function Dashboard() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <Card className="mt-5 p-5"><div className="flex items-center justify-between"><div><h2 className="font-serif text-2xl font-light">Content health</h2><p className="mt-1 text-xs text-[var(--admin-fg-muted)]">Published content available to the public website</p></div><Globe2 size={18} className="text-[var(--admin-accent)]" /></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-[var(--admin-surface-2)] p-4"><FileText size={16} /><strong className="mt-3 block text-2xl">{packages.length}</strong><span className="text-[11px] text-[var(--admin-fg-muted)]">Safari packages</span></div><div className="rounded-lg bg-[var(--admin-surface-2)] p-4"><ImageIcon size={16} /><strong className="mt-3 block text-2xl">{media.filter((item) => !item.archived).length}</strong><span className="text-[11px] text-[var(--admin-fg-muted)]">Media assets</span></div><div className="rounded-lg bg-[var(--admin-surface-2)] p-4"><Check size={16} /><strong className="mt-3 block text-2xl">{pages.filter((page) => page.status === "published").length}</strong><span className="text-[11px] text-[var(--admin-fg-muted)]">Published pages</span></div></div></Card>
    </motion.div>
  );
}
